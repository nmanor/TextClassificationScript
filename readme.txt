...
* common stylistic features (see the full list at `stylistic_features.py`)
 ...
 * To create that folder structure you can use the script `init_env.py` like so:
    ...
    ```
    or run the `init_env.py` in any python platform.
...
* To run the program from the command line, just use the command line as so:
   ...
   `main.py` file. The running may take a wile. 
...
You can create alot of config files with the `config_creator.py` script automatically.
